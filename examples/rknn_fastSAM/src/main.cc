...
int detect(char *model_path, char *image_path, char *save_image_path)
{
    ...
    for (int i = 0; i < DetectiontRects.size(); i += 6)
    {
        ...
        char text1[256];
        sprintf(text1, "%.2f", conf);
        // rectangle(SrcAddMask, cv::Point(xmin, ymin), cv::Point(xmax, ymax), cv::Scalar(255, 0, 0), 2);
        putText(SrcAddMask, text1, cv::Point(xmin, ymin + 15), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 255), 1);
    }
